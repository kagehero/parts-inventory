      ...
      {!compact ? (
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            {partName} — 現在庫 <strong className="tabular-nums text-foreground">{currentQty}</strong>
            。増減量を入力して <strong className="text-foreground">「調整」</strong>
            を押すと在庫が更新されます（上の「マスタを保存」では在庫は変わりません）。履歴は「棚卸・調整」として記録されます。
          </p>
        </div>
      ) : null}

      <form
        className={cn(compact ? "contents" : "grid gap-3")}
        onSubmit={(e) => {
          e.preventDefault();
          ...
          startTransition(async () => {
            ...
            notifyActionResult(res, "在庫を調整しました");
            if (!res.ok) {
